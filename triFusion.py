...
from generalFunctions import mergeSort

class MergeSortView(QMainWindow):
    
    def __init__(self):
        super(MergeSortView, self).__init__()
        #load the window interface file 
        loadUi('Views/triFusion.ui', self)
    ...
    def on_sortButton_clicked(self):
        if len(self.table) < 2 :
            ...
            mergeSort(self.table, 0, len(self.table)-1)
            self.updateTableView()
...
if __name__ == "__main__":
    ...
    window = MergeSortView() # Create an instance of our class
